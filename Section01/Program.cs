...
namespace Section01 {
    class Program {
        ...
        private static void Exercise2_3(List<string> names) {
            IEnumerable<string> query = names.Where(s => s.Contains("o"));
            foreach (string s in query)
                Console.WriteLine(s);
        }

        private static void Exercise2_4(List<string> names) {
            var query = names.Where(s => s.StartsWith("B")).Select(s=>s.Length);//names.Where(s=>s.StartsWith("B")).Select(s=>s.Length)
        ...
        private static void Exercise1_3(List<int> numbers) {
            ...
            foreach (int n2 in query)
                Console.WriteLine(n2);
        }
        private static void Exercise1_4(List<int> numbers) {
            List<int> list = numbers.Select(n3=>n3 * 2).ToList();
